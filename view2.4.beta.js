(function () {
    ...
    function view(app) {
        ...
        function clearBreakNode(cache) {
            each(cache, function (child) {
                if (child.nodeType) return;
                if (child.childNodes || Array.isArray(child))
                    clearBreakNode(child.childNodes || child);
...
(function () {
    ...
    var names = ['push', 'pop', 'reverse', 'shift', 'unshift','splice'];
    ...
    var observe = function (target, callSet, callGet) {
        ...
        return new _observe(target, callSet, callGet, {});
    };
