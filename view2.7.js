(function () {
    ...
    function view(app) {
        ...
        var resolver = {
            init: function (apply, scope) {
                try {
                    apply = query(apply);
                    app.view = apply[0];
                    var node = blankOut(initCompiler(init(clone(apply))))[0];
        ...
        function path(_express) {
            try {
                return _express.replace("$path:", "").replace(/(\w+)\.?/g, "['$1']").replace(/\['(\w+)'\]/, "$1");
            } catch (err) {
                console.log(err)
            }
        }
        ...
        function initCompiler(node, children) {
            return each(node, children || [], function (child, i, list) {
                ...
                switch (child.nodeType) {
                    case 1:
                        initCompiler(clone(child.childNodes), item.children);
                        break;
                    default:
        ...
        function commom(node, scope, clasNode) {
            each(node.attributes, function (child) {
                child.name.replace($express1, function (tag) {
                    try {
            ...
            if (node.nodeValue)
                node.nodeValue.replace($express1, function () {
                ...
                switch (child.node.nodeType) {
                    case 1:
                        if (child.node.hasAttribute("each")) {
                            ...
                            each(dataSource, function (item, index) {
                                ...
                                compiler(newNode, scope, clone(child.children), clasNodes);
                            ...
                            compiler(newNode, iscope, clone(child.children), clasNodes);
                    ...
                    default:
                        if ($each.test(child.node.nodeValue)) {
                            ...
                            each(dataSource, clone(child.children), function (item, index, children) {
                                var scope = Object.create(iscope || {});
                                scope[child.node.variable] = "$path:" + $path;
                                if (expreses[0]) scope[expreses[0].trim()] = "$index:" + index;
